...
class BattleshipGame:
    ...
    # Create the two player objects (two humans, or one human and one computer.
    # Then link the players so that they are aware of their opponent
    # (required to let one player attack the board of the other player.)
    ...
    # This method runs the game.  It first has the two players position their boats.
    # It then goes into a loop, in which players alternate turns until there is a winner.
    def play(self):
...
# The Board class manages the 10x10 grid that contains each player's boats.
# The board has cells, each of which is set to one of four values:
...
class Board:
    ...
    def __init__(self):
        ...
        # The hit count, used to count successful attacks. When the hit count reaches 17, the game is over.
        # This counter allows us to avoid looking for un-hit boats each turn to see if the game is over.
        # Instead, we can simply count hits as they happen and look for us to reach the total of 17.
        self.hit_count = 0
        # Adding a miss count to keep track the number of misses.
        self.miss_count = 0
    ...
    def attack(self, x, y):
        ...
        # If the cell is empty, we can mark it as a miss.
            ...
            self.grid[y][x] = " O"
            # And increase the count of the variable for misses.
            self.miss_count += 1
            return False
        # If the cell was not empty or boat, then it must have previously been attacked.
        # We can ignore a repeat attack since nothing in the grid should change.
        else:
            return False
...
class Boat:
    # Initialize the boat with a label (e.g., Destroyer) and a size (e.g., 3).
    # The position gets set later and will be undefined even after the initialization method.
    ...
    # Set the boat position.  The x,y value corresponds to the top-left position of the boat.
    # It extends either to the right or down (depending on the boat's orientation) by SIZE cells.
...
class HumanPlayer():
        ...
        while position is None:
            try:
                ...
                if not self.board.add_boat(boat):
                    # The board refused to add the boat!!!
                    # Raise an exception so that the user is forced to try a new position.
                    raise Exception
            except ValueError:
    ...
    def take_turn(self):
        ...
        # Printing the statistics of the game for the player.
        print("You have attacked", self.opponent.board.hit_count + self.opponent.board.miss_count, "times, with", self.opponent.board.hit_count, "hits and", self.opponent.board.miss_count, "misses.")
        # Get attack position.
        position = None
        while position is None:
            try:
...
class ComputerPlayer:
    # To initialize the player with a name, a empty board, and a fleet of five boats.
    def __init__(self, player_name):
        self.player_name = player_name
        self.board = Board()
        self.fleet = [Boat("Aircraft Carrier", 5), Boat("Battleship", 4), Boat("Submarine", 3), Boat("Destroyer", 3), Boat("Patrol Boat", 2)]
        self.opponent = None

    # To link the player to their opponent.
    def set_opponent(self, opponent):
        self.opponent = opponent

    # To position the fleet, one boat at a time.
    def position_fleet(self):
        for boat in self.fleet:
            self.position_boat(boat)
        # To confirm the final board to the user after their positions are decided.
        print("The computer has positioned its fleet.")

    # To position a single boat.  Helper method for positionFleet.
    def position_boat(self, boat):
        # To generate random x, y coordinates and setting up the positions for all the boats using looping function and variable.
        positioned = None
        while positioned is None:
            orientation = random.choice('hv')
            x = int(random.randrange(10))
            y = int(random.randrange(10))
            boat.set_orientation(orientation)
            boat.set_position(x,y)
            # To change the variable after the positioning is over.
            if self.board.add_boat(boat):
                positioned = 'done'
                return positioned

    # This function is to manage a single turn for the player.
    def take_turn(self):
        # To generate random x, y coordinates to attack the player.
        x = int(random.randrange(10))
        y = int(random.randrange(10))

        # To perform an attack
        hit_boat = self.opponent.board.attack(x, y)
        if hit_boat:
            print("The computer attacked",(x,y),"and hit a boat!")
        else:
            print("The computer attacked",(x,y),"and missed.")

        # To return true if the opponent has been defeated.  Otherwise, false.
        if self.opponent.board.is_defeated():
            return True
        else:
            return False